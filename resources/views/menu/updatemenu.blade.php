...
<form  id="formEdit" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="kode" value="{{ $menumakanan->kode}}">
	<!-- Field Nama, Type Varchar/String -->
	<div class="form-group">
		<label for="nama">Nama</label>
		<input type="text" name="nama" class="form-control" id="nama" value="{{ $menumakanan->nama}}" required>
	...
	<div class="form-group">
		...
		<div class="input-group mb-3">
			<input type="number" name="harga" class="form-control" id="harga" value="{{ $menumakanan->harga}}" required>
	...
	<div class="form-group">
		...
		<div class="form-check">
			...
			<input type="radio" class="form-check-input" name="status" id="statustrue" value="1" {{ $menumakanan->status==1?'checked':''}}>
				Yes </label>
			<label class="form-check-label mr-4">
			<input type="radio" class="form-check-input" name="status" id="statusfalse" value="0" {{ $menumakanan->status==0?'checked':''}}>
				No</label>
	...
	<div class="form-group">
		...
		<img id="photo" class="img-thumbnail" style="width: 100px; height: 75px;" src="{{asset('uploads/image/MenuMakanan_photo/'.$menumakanan->photo)}}" alt="your image" /></br></br>
		<input type="file" class="form-control-file" accept="image/jpeg, image/png" id="photo" name="photo" onchange="readURLImage(this,'#photo');">
		<span id="errorphoto" class="text-danger small"></span>
	</div>
	<p id="messageInfo" class="text-result"></p>
	<!-- Modal footer -->
	<div class="modal-footer">
		<button id="addMenuMakanan" class="btn btn-success">Save</button>
...
<script>
	...
	$(document).ready(function() {
		$('#formEdit').on('submit', function(e) {
			...
			$.ajax({
				method: "POST",
				url: "/menumakanan/update/{{$menumakanan->kode}}",
